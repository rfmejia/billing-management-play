/**
 * Created by juancarlos.yu on 2/5/15.
 */
var documentEdit = angular.module("module.mailbox");

documentEdit.controller("controller.edit", ["$scope", "$state", "documentsService", "tenantsList", "document", "documentsResponse", "$filter", "$modal", "moment", "toaster",
        function($scope, $state, documentsService, tenantsList, documentsResponse, $filter,  $modal, moment, toaster){
            /** Tenants list **/
            $scope.tenants = tenantsList.tenants;
            /** Previous months template **/
            $scope.previous = documentsResponse.breakdown.previous;
            /** This months template **/
            $scope.thisMonth = documentsResponse.breakdown.thisMonth;
            /** Summary template **/
            $scope.summary = documentsResponse.summary;
            ...
            $scope.onSaveClicked = function(billingForm) {
                preparePostData();
                documentsService.editDocument($scope.documentId.id, $scope.postTemplate)
            ...
            /**
             * DOM adds a few buttons and changes endpoints when we are in editing mode.
             * @returns {boolean}
             */
            $scope.isInEditingMode = function() {
                return $scope.documentId.id != null;
            };
            ...
            $scope.prepareDeleteModal = function() {
                ...
                $scope.modalPositive = function(response) {
                    documentsService.deleteDocument($scope.documentId.id)
                        .then(function(response) {
            ...
            /**
             * Handles the submit modal
             */
            $scope.prepareSubmitModal = function() {
                $scope.negativeButton.type = "btn-default";
                $scope.negativeButton.message = "Cancel";

                $scope.positiveButton.type = "btn-primary";
                $scope.positiveButton.message = "Submit";

                $scope.modal.title = "Submit document";
                $scope.modal.message = "Send document for checking?";
